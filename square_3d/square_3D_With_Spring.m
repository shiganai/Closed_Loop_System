...
syms k real
syms x_Fixed y_Fixed z_Fixed real
...
U = (p1(3) * m1 + p2(3) * m2 + p3(3) * m3) * g...
    + 1/2 * k * ((p3 - [x_Fixed, y_Fixed, z_Fixed]) * (p3 - [x_Fixed, y_Fixed, z_Fixed])')...
    ;
L = T - U;
...
tic
variables = [ddth1, ddth2, ddth3, ddphi1, ddphi2, ddphi3];
...
job = createJob(c);
createTask(job, @matlabFunction, 1,{ddth1_Eq, ddth2_Eq, ddth3_Eq, ddphi1_Eq, ddphi2_Eq, ddphi3_Eq, 'file', 'find_Dds_With_Spring.m', 'outputs', {'ddth1', 'ddth2', 'ddth3', 'ddphi1', 'ddphi2', 'ddphi3'}});
submit(job)
job.Tasks
