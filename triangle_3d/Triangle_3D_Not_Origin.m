...
p3 = p0;
p2_Prime = p3 + l_Arm * [sin(th3_Pre) * cos(phi3_Pre), sin(th3_Pre) * sin(phi3_Pre), cos(th3_Pre)];

restraint = p2_Prime' == p2';
...
p2 = formula(p2);
p3 = formula(p3);
p2_Prime = formula(p2_Prime);

v1 = diff(p1);
v2 = diff(p2);
v3 = diff(p3);
v2_Prime = diff(p2_Prime);

T = 1/2 * m1 * (v1 * v1') ...
    + 1/2 * (m2/2) * (v2 * v2') ...
    + 1/2 * m3 * (v3 * v3') ...
    + 1/2 * (m2/2) * (v2_Prime * v2_Prime') ...
    ;
U = (p1(3) * m1 ...
    + p2(3) * m2/2 ...
    + p3(3) * m3....
...
M = m1 * v1 + m2/2 * v2 + m3 * v3 + m2/2 * v2_Prime;
d_M = diff(M, t);
...
equations = [
    restraint;
    -functionalDerivative(L, th1_Pre) ...
    == tau1 + (f_X * diff(p2(1), th1_Pre) + f_Y * diff(p2(2), th1_Pre) + f_Z * diff(p2(3), th1_Pre)) + (-f_X * diff(p2_Prime(1), th1_Pre) + -f_Y * diff(p2_Prime(2), th1_Pre) + -f_Z * diff(p2_Prime(3), th1_Pre));
    -functionalDerivative(L, th2_Pre) ...
    == tau2 + (f_X * diff(p2(1), th2_Pre) + f_Y * diff(p2(2), th2_Pre) + f_Z * diff(p2(3), th2_Pre)) + (-f_X * diff(p2_Prime(1), th2_Pre) + -f_Y * diff(p2_Prime(2), th2_Pre) + -f_Z * diff(p2_Prime(3), th2_Pre));
    -functionalDerivative(L, th3_Pre) ...
    == tau3 + (f_X * diff(p2(1), th3_Pre) + f_Y * diff(p2(2), th3_Pre) + f_Z * diff(p2(3), th3_Pre)) + (-f_X * diff(p2_Prime(1), th3_Pre) + -f_Y * diff(p2_Prime(2), th3_Pre) + -f_Z * diff(p2_Prime(3), th3_Pre));
    -functionalDerivative(L, phi1_Pre) ...
    == 0 + (f_X * diff(p2(1), phi1_Pre) + f_Y * diff(p2(2), phi1_Pre) + f_Z * diff(p2(3), phi1_Pre)) + (-f_X * diff(p2_Prime(1), phi1_Pre) + -f_Y * diff(p2_Prime(2), phi1_Pre) + -f_Z * diff(p2_Prime(3), phi1_Pre));
    -functionalDerivative(L, phi2_Pre) ...
    == 0 + (f_X * diff(p2(1), phi2_Pre) + f_Y * diff(p2(2), phi2_Pre) + f_Z * diff(p2(3), phi2_Pre)) + (-f_X * diff(p2_Prime(1), phi2_Pre) + -f_Y * diff(p2_Prime(2), phi2_Pre) + -f_Z * diff(p2_Prime(3), phi2_Pre));
    -functionalDerivative(L, phi3_Pre) ...
    == 0 + (f_X * diff(p2(1), phi3_Pre) + f_Y * diff(p2(2), phi3_Pre) + f_Z * diff(p2(3), phi3_Pre)) + (-f_X * diff(p2_Prime(1), phi3_Pre) + -f_Y * diff(p2_Prime(2), phi3_Pre) + -f_Z * diff(p2_Prime(3), phi3_Pre));
    ];
...
job = createJob(c);
createTask(job, @matlabFunction, 1,{ddth1_Eq, ddth2_Eq, ddth3_Eq, ddphi1_Eq, ddphi2_Eq, ddphi3_Eq, 'file', 'find_Dds_Not_Origin.m', 'outputs', {'ddth1', 'ddth2', 'ddth3', 'ddphi1', 'ddphi2', 'ddphi3'}});
submit(job)
job.Tasks
